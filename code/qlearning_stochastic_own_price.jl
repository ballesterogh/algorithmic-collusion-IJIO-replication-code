...
module qlearning_stochastic_own_price

    export simulate_game_own_price

    using Random, QuantEcon

    const MASTER_SEED = 111 

        function get_actions(game::Main.init_stochastic.model, Q::Array{Float32,6}, a::Vector{Int8}, cost::Vector{Int64},t::Int64, i::Int64, j::Int64,s::Int64, rng::AbstractRNG)::Vector{Int8}
            # Action module
            a_current = Int8.(zeros(game.n));
            a_current[j]=a[j];
            pr_explore::Float64 = exp(- t * game.beta);
            e = pr_explore > rand(rng);
            a_current[i] = e ? Int8(rand(rng, 1:game.dim_A)) : Int8(argmax(Q[i, a[i], a_current[j], cost[t], cost[t-1], :]));   
            return a_current
        end

        
        function update_Q(game::Main.init_stochastic.model, Q::Array{Float32,6}, t::Int64, i::Int64, j::Int64, d::Int64, profit_matrix::Matrix{Float32}, cost::Vector{Int64}, a::Array{Int8,2}, stable::Array{Int64,1})::Tuple{Array{Float32,6},Array{Int64,1}}
            # The state of firm i at period t is given by: s_it = (p_it-1 p_jt-1, c_t-1, c_t)

            # Learning module
            old_value = Q[i, a[t-3,i], a[t-2,j], cost[t-2], cost[t-3], a[t-2,i]];
            old_argmax = argmax(Q[i, a[t-3,i], a[t-2,j], cost[t-2], cost[t-3],:]);
        
            max_q = maximum(Q[i, a[t-2,i], a[t-1,j], cost[t-1], cost[t-2],:])  # max Q-value
            new_value = profit_matrix[t-2,i] + game.delta[d] * profit_matrix[t-1,i] + game.delta[d]^2 * max_q;
            Q[i, a[t-3,i], a[t-2,j], cost[t-2], cost[t-3], a[t-2,i]] = (1 - game.alpha) * old_value + game.alpha * new_value;
        
            # Check stability
            new_argmax = argmax(Q[i, a[t-3,i], a[t-2,j], cost[t-2], cost[t-3], :]);
    ...
    function simulate_game_own_price(game::Main.init_stochastic.model)::Tuple{Array{Float32,9}, Array{Float32,5}, Array{Int8,5}, Array{Int8,4},Array{Int64,3},Array{Int64,8}}
        Q_all::Array{Float32,9} = zeros(game.n, game.dim_A, game.dim_A, game.dim_C, game.dim_C, game.dim_A, game.S, game.dim_rho, game.dim_delta);
        PI = Array{Float32,5}(undef, game.tstable, 2, game.S, game.dim_rho, game.dim_delta);
        a_converge = Array{Int8,5}(undef, game.tstable, 2,game.S, game.dim_rho, game.dim_delta);
        ...
        bf::Array{Int64,8}=zeros(Int64, game.n, game.dim_A,game.dim_A, game.dim_C, game.dim_C, game.S, game.dim_rho, game.dim_delta);
        ...
        for d=1:game.dim_delta
            for r=1:game.dim_rho
                for s=1:game.S
                    ...
                    Q::Array{Float32,6} = zeros(game.n, game.dim_A, game.dim_A, game.dim_C, game.dim_C, game.dim_A);
                    ...
                    a[1:t0,:] = Int8.(rand(rng, 1:game.dim_A, t0, game.n)); # Initial state
            ...
            for r=1:game.dim_rho
                for s=1:game.S
                    for n=1:game.n
                        for c=1:game.dim_C
                            for c1=1:game.dim_C
                                for a_j=1:game.dim_A
                                    for a_i=1:game.dim_A
                                        bf[n,a_i,a_j,c,c1,s,r,d]= argmax(Q_all[n,a_i, a_j,c,c1,:,s,r,d]);
                                    end
                                end
                            end
                        end
