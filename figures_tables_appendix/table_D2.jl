...
@load joinpath(data_dir, "outcomes_stochastic_uncertainty.jld2") game Q_matrix profits actions cost t_final bf;

include("compute_profit_normalized.jl") 

π_bar_comp = 0.045; 
π_bar_monop = (1/3) * (game.profit_monopoly[1]/2) + (1/3) * (game.profit_monopoly[2]/2) + (1/3) * (game.profit_monopoly[3]/2);
...
writedlm("table_D2.txt", output_lines)
